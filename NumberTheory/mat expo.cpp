...
void multiply (long long A[N][N], long long B[N][N]){long long R[N][N];
// Multiply A and B and store result in R.
for (int i = 0; i < N; i++){for (int j = 0; j < N; j++){
R[i][j] = 0;for (int k = 0; k < N; k++){R[i][j] = (R[i][j] + A[i][k] * B[k][j]) % M;}}}
// Copy contents of R in A.
for (int i = 0; i < N; i++){for (int j = 0; j < N; j++){A[i][j] = R[i][j];}}}
// Raise matrix A to the power of n in O(log n).
void power_matrix (long long A[N][N], ll n){long long B[N][N];// B = Identity Matrix.
...
long long solve_recurrence (long long A[N][N], long long B[N][1], ll n){//Base Cases.
if (n < N)return B[N - 1 - n][0];// A = A ^ (n - N + 1).
power_matrix (A, n - N + 1);long long result = 0;for (int i = 0; i < N; i++)
...
long long B[N][1] = {{3}, {2}, {1}, {3}};ll n;cin >> n;if (n <= 2){
cout << n + 1 << endl;return ;}long long R_n = solve_recurrence (A, B, n + 1);
cout << R_n << endl;}
